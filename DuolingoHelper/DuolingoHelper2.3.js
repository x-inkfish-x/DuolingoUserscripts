...
DuolingoHelper.prototype.requestLanguageTrackingData = function (callback) {
    if (callback) {
        ...
        var args = {
            ...
            url: "/2017-06-30/users/" + userId + "?fields=courses"
        }

        this.makeGetRequest(args);
    }
}
...
DuolingoHelper.prototype.startListenForContentUpdate = function () {
    this.observer = new MutationObserver(function (mutations) {
        ...
        for (var mutation of mutations) {
            for (var node of mutation.addedNodes) {
                if (node.localName == 'div') {
                    ...
                    foundElements = node.getElementsByClassName(this.languageDropdownEntryClass);
                    if (foundElements && foundElements.length > 0) {
                        languageFields = languageFields.concat(Array.from(foundElements));
                    }
        ...
        if (this.onRevealLanguages && languageFields.length > 0) {
            this.requestLanguageTrackingData(function (languages) {
                if (languages) {
                    var languageMap = this.mappifyLanguageArray(languages.courses);
                    for (var languageField of languageFields) {
                        this.getLanguageKey(languageField);
                        this.onRevealLanguages(languageField,
                            languageMap[this.getLanguageKey(languageField)]);
                    }
                }
            }.bind(this))
        }
...
DuolingoHelper.prototype.mappifyLanguageArray = function (langArr) {
    var langMap = new Map;
    if (langArr) {
...
DuolingoHelper.prototype.getLanguageKey = function (node) {
    for (var key in node) {
        if (key.startsWith("__reactInternalInstance$")) {
            var prop = node[key];
